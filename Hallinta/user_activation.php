<?php
    ...
    if($token != "") {
        $sqlQuery = mysqli_query($link, "SELECT * FROM users WHERE token = '$token' ");
        $countRow = mysqli_num_rows($sqlQuery);

        if($countRow == 1){
            while($rowData = mysqli_fetch_array($sqlQuery)){
                $is_active = $rowData['verified'];
                  if($is_active == 0) {
                     $update = mysqli_query($link, "UPDATE users SET verified = '1' WHERE token = '$token' ");
                       if($update){
